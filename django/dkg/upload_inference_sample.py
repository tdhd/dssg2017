...
if __name__ == "__main__":
    ...
    parsed_articles = [{'title': row.joined_titles, 'abstract': row.joined_abstracts} for _, row in ris_df.iterrows()]

    headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
    payload = {
        'articles': parsed_articles
    }
    r = requests.post("http://---:---@localhost:8000/cancer/json/inference", json=payload, headers=headers)
    print r.text
